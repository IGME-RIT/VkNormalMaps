...
void Demo::prepare_descriptor_pool()
{
	...
	// we will have five image sampler in the entire program
	// 3 entities have just color textures
	// 1 entity has 2 textures: color and normal
	type_counts[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	type_counts[1].descriptorCount = 5;
...
void Demo::build_secondary_cmd()
{
	...
	// Draw the cat and dog, which both use the pipeBasic pipeline
	catEntity->Draw(cmd, pipeBasic->pipeline_layout);
	dogEntity->Draw(cmd, pipeBasic->pipeline_layout);

	// Bind our pipeline, let Vulkan know that it is a GRAPHICS pipeline.
	// There are other types of pipelines, so we need to specify GRAPHICS.
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeBumpy->pipeline);
	
	// Draw the building, which will use the pipeBumpy pipeline
	buildingEntity->Draw(cmd, pipeBumpy->pipeline_layout);
...
void Demo::prepare()
{
	...
	if (firstInit)
	{
		...
		// We give true, to calculate tangents (for nomral maps),
		// or we give false, to skip calculating tangents (everything else)

		// Load all the OBJ files
		catMesh = new Mesh("../../../Assets/kitten.3Dobj", this, false);
		dogMesh = new Mesh("../../../Assets/dog.3Dobj", this, false);
		rockMesh = new Mesh("../../../Assets/building.3Dobj", this, true);
		skyMesh = new Mesh("../../../Assets/skybox.3Dobj", this, false);
		...
		rockColor = new Texture("../../../Assets/BrickColor.png", this);
		rockNormal = new Texture("../../../Assets/BrickNormal.png", this);
	...
	if (firstInit)
	{
		...
		delete rockColor->textureCPU;
		delete rockNormal->textureCPU;
...
Demo::~Demo()
{
	...
	delete rockColor->textureGPU;
	delete rockNormal->textureGPU;
